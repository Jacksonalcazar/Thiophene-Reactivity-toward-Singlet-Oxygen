...
def process_files(alpha_carbons_pairs: List[List[int]]) -> float:
    ...
    for pair in alpha_carbons_pairs:
        ...
        F = math.log10(abs(XM * (S + 30.75 * sum_s_plus) + (0.11686 / sum_s_plus)))
        Y = 8.612 * (charges_n.get(index1, 0) + charges_n.get(index2, 0)) - 76.0* F + 53.2494
        M = 0.5251*Y + 8.646
        Z += 10**M
        log_k_k2T = math.log10(Z)
...
def print_result(y_value):
    ...
    print("*                                       *")
    print("*****************************************")
    print("Note: This estimate is derived from a theoretical context in an implicit methanol environment.")
    print("      The reactivity is relative to the rate constant of 2-tert-butylthiophene (k2T).\n")
